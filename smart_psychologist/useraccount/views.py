...
class UserRegistration(View):  
    def get(self,request):
        if not request.user.is_authenticated:
            ...
            return render(request,'useraccount/signup.html',{'forms':forms})
        else:
...
class UserProfile(View):
    def get(self,request):
        ...
        return render(request,'useraccount/profile.html',{'customar_profile':customar_profile,'pro_active':'bg-primary text-white'})


# def profiletest(request):
#     if request.user.is_authenticated:
#         pi = Customer.objects.get(user=request.user)
#         form = CustomerProfileForm(instance=pi)
#         return render(request,'useraccount/profileUP.html',{'form':form})
#     else:
#         return redirect('/')
...
class UserProfileUpdate(View):
    ...
    def post(self,request):
        ...
        if pi:
            pi = Customer.objects.get(user=user)
            form = CustomerProfileForm(request.POST,request.FILES,instance=pi)
            if form.is_valid():
                form.save()
...
def userAddress(request):
    ...
    return render(request,'useraccount/address.html',{'add':add,'ad_active':'bg-primary text-white'})
...
class AddDeliveryAddress(View):
    def get(self,request):
        ...
        return render(request,'useraccount/adddeliveryaddress.html',{'form':form,'dlyAd_active':'bg-primary text-white'})
    def post(self,request):
        ...
        if form.is_valid(): 
            ...
            city = form.cleaned_data['city']
            zip_code = form.cleaned_data['zip_code']
            adrs_data = DeliveryAddress(user=user, home_address=home_address,division=division,district=district,city=city,zip_code=zip_code)
            adrs_data.save()
